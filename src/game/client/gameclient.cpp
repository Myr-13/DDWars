...
int CGameClient::GetNearestTee() {
	...
	for (int i = 0; i < MAX_CLIENTS; i++) {
		...
		float distnce = distance(m_Snap.m_aCharacters[i].m_Position, m_LocalCharacterPos);
		if (distnce < dist) {
			dist = distnce;
			near = i;
		}
	...
	if(distance(m_Snap.m_aCharacters[near].m_Position, m_LocalCharacterPos) > base_dist)
		near = -1;
...
int CGameClient::GetTile(int PosX, int PosY) {
	return m_GameWorld.m_pCollision->GetTile(PosX, PosY);
}

int CGameClient::GetFTile(int PosX, int PosY)
{
	return m_GameWorld.m_pCollision->GetFTile(PosX, PosY);
}

bool CGameClient::IsFreezeTile(int PosX, int PosY) {
	return (GetTile(PosX, PosY) == TILE_FREEZE || GetFTile(PosX, PosY) == TILE_FREEZE);
}

bool CGameClient::IsHookTile(int PosX, int PosY)
{
	return (GetTile(PosX, PosY) == TILE_SOLID || GetFTile(PosX, PosY) == TILE_SOLID);
}

void CGameClient::Hook() { m_pControls->m_InputData->m_Hook = !m_pControls->m_InputData->m_Hook; }
void CGameClient::Right() {
	m_pControls->m_InputDirectionLeft[0] = 0;
	m_pControls->m_InputDirectionRight[0] = 1;
}
void CGameClient::Left() {
	m_pControls->m_InputDirectionLeft[0] = 1;
	m_pControls->m_InputDirectionRight[0] = 0;
}

void CGameClient::SetMousePos(vec2 pos) { m_pControls->m_MousePos[0] = pos; }

void CGameClient::ClientUpdate()
{
	vec2 pos = m_LocalCharacterPos;
	...
	{
		for (int j = 0; j < 5; j++) {
			if(IsHookTile(pos.x + i * 32, pos.y + j * 32))
			{
...
// MClient
void CGameClient::BotAimDistance(IConsole::IResult *pResult, void *pUserData)
{
	CGameClient *client = (CGameClient *)pUserData;

	client->BotMaxDistance = pResult->GetInteger(0);
}

void CGameClient::BotAdminID(IConsole::IResult *pResult, void *pUserData)
{
	CGameClient *client = (CGameClient *)pUserData;

	int x = client->m_LocalCharacterPos.x;
	int y = client->m_LocalCharacterPos.y;

	char aBuf[256];

	for(int x2 = 0; x2 < 300; x2++)
	{
		for(int y2 = 0; y2 < 300; y2++)
		{
			str_format(aBuf, 256, "x: %i y: %i tile: %i", x + x2, y + y2, client->GetTile(x + x2, y + y2));
			client->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "mclient", aBuf);
		}
	}
}
